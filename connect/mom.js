  ...
  try {
    ...
    if (isGroup && global.db.chats[m.from].antilink) {
      if (budy.match(`chat.whatsapp.com`)) {
        m.reply(`_*Link Grup Lain Terdeteksi*_\nlu bakal gua kick dek`)
        ...
        if (isgclink) return m.reply(`gajadi, ternyata link grup ini hehe`)
        if (isAdmin) return m.reply(`gajadi, ternyata lu admin jirt`)
        if (isOwner) return m.reply(`sorry king egi, anda adalah owner saya`)
