...
function bind(mom) {
    ...
    mom.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    mom.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    mom.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    mom.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   mom.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    mom.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    mom.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    mom.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    mom.ev.on('chats.update', updates => {
    ...
    mom.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    mom.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**mom.ev.on('messages.update', updates => {
    ...
    mom.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    mom.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    mom.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (mom === null || mom === void 0 ? void 0 : mom.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (mom === null || mom === void 0 ? void 0 : mom.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (mom === null || mom === void 0 ? void 0 : mom.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return mom === null || mom === void 0 ? void 0 : mom.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (mom === null || mom === void 0 ? void 0 : mom.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (mom === null || mom === void 0 ? void 0 : mom.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (mom === null || mom === void 0 ? void 0 : mom.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (mom === null || mom === void 0 ? void 0 : mom.messageInfo(remoteJid, id))
                if (msg) {
